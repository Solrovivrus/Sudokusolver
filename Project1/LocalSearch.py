...
class LocalSearch:
    ...
    def simulated_Anneal(original, temp):
        ...
        new_Puzzle = ls.assignRandomValues(original_puzzle)
        print(puzzles.print_puzzle(new_Puzzle))
        old_conflicts = ls.countConflict(new_Puzzle)
        print("number of starting conflicts " + str(old_conflicts))
        solved = False

        while not solved:
            switch = ls.chooseConflicts(new_Puzzle, original_puzzle)
...
def testing():
    ...
    original = easy[5]
    print(puzzles.print_puzzle(original))

    
    
    solved = ls.simulated_Anneal(original, 100)
